-- Reconcile the durable agent list at the terminal boundary.
-- This compensates for transport-layer truncation while preserving a closed allowlist.
...
begin
  ...
  foreach v_agent in array coalesce(p_activated_agents, '{}'::text[]) loop
    ...
    if v_agent in ('ceo','cfo','coo','cio','cmo','cro','reliability-sre','runtime-mechanic','memory-curator','incident-auditor')
       and not (v_agent = any(v_agents))
       and cardinality(v_agents) < 6 then
  ...
  if v_posts is not null and jsonb_typeof(v_posts) = 'array' then
    for v_agent in
      select nullif(btrim(item->>'agent'), '')
      from jsonb_array_elements(v_posts) item
    loop
      if v_agent in ('ceo','cfo','coo','cio','cmo','cro','reliability-sre','runtime-mechanic','memory-curator','incident-auditor')
         and not (v_agent = any(v_agents))
         and cardinality(v_agents) < 6 then
  ...
  if v_recovered > 0 then
    v_result := jsonb_set(
      ...
      jsonb_build_object(
        'version','1.0.0',
        'recovered_from_validated_telegram_posts',v_recovered,
        'durable_agent_count',cardinality(v_agents),
  ...
  set status=v_status,result=v_result,locked_at=null,locked_by=null,updated_at=now()
  where id=p_task_id;

  update public.af_runs
  set status=v_status,output=v_result,activated_agents=v_agents,
  ...
  values(v_run_id,p_task_id,'RUN_'||v_status,'runtime-truth-gate',
         case when v_status='COMPLETE' then 'CONFIRMED' else 'BLOCKER' end,v_result);
