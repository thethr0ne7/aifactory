...
const result={checked_at:new Date().toISOString(),correlation_id:correlationId,delivered_stages:requiredStages,pass_handoffs:handoffs.filter((x)=>x.gate_status==='PASS').length,evidence_producers:producers.size,expected_operational_contributors:expectedOperational.length,child:{candidate_id:child.candidate_id,name:child.name,generation:child.generation,role:child.role,state:child.state,autonomy_level:child.autonomy_level},production_authority_granted:false,publication_attempted:false};
await fs.mkdir('artifacts',{recursive:true});await fs.writeFile('artifacts/synergy-birth.json',JSON.stringify(result,null,2)+'\n');
console.log(`SYNERGY_BIRTH_OK stages=${requiredStages.length} handoffs=4 contributors=${producers.size} child=${child.candidate_id}`);
