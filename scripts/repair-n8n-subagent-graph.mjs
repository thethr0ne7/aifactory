...
await request({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'ai-factory-subagent-graph-repair', version: '1.0.0' } } });
...
// Required by the MCP contract before building/replacing Agent configs.
  ...
  for (const parentName of modified.reverse()) {
    try {
      ...
      await tool('mutate_agent', {
        agentId: resolved[parentName].id,
        baseConfigHash: current.configHash,
        operation: { type: 'config.replace', config: parentSnapshots.get(parentName).config },
      });
    } catch (error) {
      console.error(`ROLLBACK_FAILED parent=${parentName} error=${String(error?.message || error).slice(0, 500)}`);
    }
  }
}
...
try {
  for (const [parentName, children] of Object.entries(topology)) {
    ...
    await tool('mutate_agent', {
      ...
      operation: { type: 'config.replace', config: nextConfig },
    });
...
for (const [parentName, children] of Object.entries(topology)) {
  ...
  verification.push({
    parent: parentName,
    parent_agent_id: resolved[parentName].id,
    child_count: snapshot?.config?.subAgents?.agents?.length || 0,
    expected_children: children.map(([name]) => name),
    persisted: (snapshot?.config?.subAgents?.agents?.length || 0) === 4,
  });
}

await fs.mkdir('artifacts', { recursive: true });
await fs.writeFile('artifacts/n8n-subagent-graph.json', JSON.stringify({ checked_at: new Date().toISOString(), verification, published_or_unpublished_state_changed: false, publication_attempted: false, root_of_trust_mutation_attempted: false }, null, 2) + '\n');
