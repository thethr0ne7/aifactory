...
function walk(v,path='$',out=[]){if(!v||typeof v!=='object')return out;for(const [k,val] of Object.entries(v)){const p=`${path}.${k}`;if(/sub.?agents?|configHash|agentId|tools|memory|config|task|skill|custom/i.test(k)){out.push({path:p,type:Array.isArray(val)?'array':typeof val,keys:val&&typeof val==='object'&&!Array.isArray(val)?Object.keys(val):undefined,length:Array.isArray(val)?val.length:undefined});}if(val&&typeof val==='object')walk(val,p,out);}return out;}
await req({jsonrpc:'2.0',id:1,method:'initialize',params:{protocolVersion:'2025-06-18',capabilities:{},clientInfo:{name:'ai-factory-agent-graph-probe',version:'1.1.0'}}});
...
const relevant=tools.filter(t=>/agent|delegate|handoff|tool|task|skill/i.test(`${t.name} ${t.description||''}`)).map(t=>({name:t.name,description:t.description,inputSchema:t.inputSchema}));
const agent=structured(await req({jsonrpc:'2.0',id:3,method:'tools/call',params:{name:'get_agent',arguments:{agentId:supervisorId}}}));
console.log(JSON.stringify({tool_count:tools.length,relevant_tools:relevant,supervisor_shape:walk(agent),supervisor_config:agent?.config||null},null,2));
