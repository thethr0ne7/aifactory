...
async function runTask(taskId:string,worker:string){
  ...
  try{
    ...
    const token=await runtimeSecret("n8n_mcp_token");
    const out=await callAgent(token,working.assigned_agent_ref,taskPrompt(ctx));
    ...
    await activity(working.session_id,working.id,status==="BLOCKED"?"BLOCKER":"AGENT_MESSAGE",working.assigned_agent_ref,null,ctx.agent?.name||working.assigned_agent_ref,message,{evidence_class:evidenceClass,status,executor:"supabase-edge-hot"});
...
async function context(task:any){
  const [{data:session,error:sError},{data:events,error:eError},{data:tasks,error:tError},{data:candidates,error:cError}]=await Promise.all([
    ...
    db.from("af_agent_candidates").select("candidate_id,n8n_agent_id,name,role,state,autonomy_level").in("state",["SPAWNED","TRAINING","EVALUATING","REPAIRING","CANDIDATE","PROMOTED"]).order("generation").limit(40)
  ]);
...
function taskPrompt(ctx:any){return[
  ...
  `YOUR_AGENT=${ctx.task.assigned_agent_ref}`,
...
async function callAgent(token:string,candidateId:string,prompt:string){
  ...
  let rpc=1;
  async function mcp(message:any){const r=await fetch(N8N_MCP_URL,{method:"POST",headers:{authorization:`Bearer ${token}`,"content-type":"application/json",accept:"application/json, text/event-stream"},body:JSON.stringify(message)});const text=await r.text();const payload=parsePayload(text,r.headers.get("content-type")||"");if(!r.ok||payload?.error)throw new Error(`MCP_${r.status}_${JSON.stringify(payload).slice(0,800)}`);return payload;}
  await mcp({jsonrpc:"2.0",id:rpc++,method:"initialize",params:{protocolVersion:"2025-06-18",capabilities:{},clientInfo:{name:"ai-factory-edge-hot-runtime",version:"1.0.0"}}});
...
function clean(v:any,max=4000){return String(v??"").replace(/[\u0000\r]+/g," ").trim().slice(0,max);}function uuid(v:any,n:string){const s=String(v??"");if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(s))throw new Error(`invalid_${n}`);return s;}function constantTimeEqual(a:string,b:string){if(a.length!==b.length)return false;let d=0;for(let i=0;i<a.length;i++)d|=a.charCodeAt(i)^b.charCodeAt(i);return d===0;}async function safeJson<T>(r:Request){try{return await r.json() as T;}catch{return{} as T;}}function safeError(e:any){return(e instanceof Error?e.message:String(e)).replace(/[\r\n]+/g," ").slice(0,1200);}function json(v:any,s=200){return new Response(JSON.stringify(v),{status:s,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}});}
